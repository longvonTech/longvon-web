...
export class IntelligenceService {
  ...
  async crawlIndustryTopic(topic: string, keywords: string[]) {
    ...
    for (const article of articles) {
      ...
      try {
        const aiSummary = await this.qwen.summarizeArticle(article.title, article.summary ?? '', topic);
        const relevance = await this.qwen.assessRelevance(article.title, aiSummary, keywords);
        await this.repo.createIndustryEvent({
          title: article.title, sourceUrl: article.url, sourceName: article.source,
          topic, relevance, rawContent: article.summary, aiSummary, status: 'processed',
        });
  ...
  async crawlCompetitor(competitor: string) {
    ...
    for (const article of articles) {
      ...
      seen.add(article.title);
      try {
        const aiSummary = await this.qwen.summarizeArticle(article.title, article.summary ?? '', `竞品${competitor}`);
        const eventType = this.classifyCompetitorEvent(article.title);
        await this.repo.createCompetitorEvent({
          competitor, eventType, title: article.title,
          sourceUrl: article.url, sourceName: article.source,
          rawContent: article.summary, aiSummary, impact: 'medium', status: 'processed',
        });
  ...
  private classifyCompetitorEvent(title: string): string {
    ...
    if (t.includes('new') || t.includes('launch') || t.includes('introduce') || t.includes('新品') || t.includes('发布')) return 'product_update';
    if (t.includes('research') || t.includes('study') || t.includes('tech') || t.includes('算法') || t.includes('技术')) return 'tech_upgrade';
    if (t.includes('hire') || t.includes('job') || t.includes('招聘') || t.includes('团队')) return 'hiring';
    if (t.includes('price') || t.includes('pricing') || t.includes('价格') || t.includes('折扣')) return 'pricing';
    if (t.includes('review') || t.includes('report') || t.includes('media') || t.includes('媒体') || t.includes('报道')) return 'media_coverage';
    return 'other';
  }
    ...
    for (const { topic, keywords } of this.RESEARCH_TOPICS) {
      for (const kw of keywords.slice(0, 1)) {
        ...
        for (const paper of papers.slice(0, 4)) {
          try {
            const aiSummary = await this.qwen.summarizeArticle(paper.title, paper.summary ?? '', `${topic}医学研究`);
            await this.repo.createResearchPaper({
              title: paper.title, abstract: paper.summary,
              sourceUrl: paper.url, topic, aiSummary, relevance: 'medium', status: 'summarized',
            });
            await this.sleep(1500);
          } catch {}
        }
  ...
  async generateDailyReports() {
    ...
    if (industryEvents.length > 0) {
      const { title, content, highlights } = await this.qwen.generateReport('行业', industryEvents, '今日');
      await this.repo.createIndustryReport({
        reportType: 'daily', periodStart: yesterday, periodEnd: today,
        title, content, highlights, eventCount: industryEvents.length,
      });
    }
    ...
    if (competitorEvents.length > 0) {
      ...
      await this.repo.createCompetitorReport({
        competitor: 'all', reportType: 'daily', periodStart: yesterday, periodEnd: today,
        title, content, insights, eventCount: competitorEvents.length,
      });
    }
  ...
  private async generateCompetitorReport(events: any[]) {
    const eventList = events.map((e, i) => `${i+1}. [${e.competitor}] ${e.title}${e.aiSummary ? '：' + e.aiSummary : ''}`).join('\n');
    ...
    let insights: string[] = [];
    try {
      const h = await this.qwen.complete(`从以下竞品报告中提取3条最重要的竞品洞察，JSON数组格式：\n${content.slice(0,800)}`);
  ...
  async triggerManual(engine: 'industry' | 'competitor' | 'research' | 'report') {
    switch (engine) {
      case 'industry':  return this.runIndustryIntelligence();
      case 'competitor': return this.runCompetitorIntelligence();
      case 'research':  return this.runResearchIntelligence();
      case 'report':    return this.generateDailyReports();
    }
  }
