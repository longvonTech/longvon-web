...
const KEYWORD_SEEDS = [
  // 核心产品词
  { cluster: '智能戒指', topic: 'smart_ring', keywords: ['智能戒指', '健康戒指', 'smart ring', '智能可穿戴戒指', '睡眠监测戒指', 'Ring1C', 'Oura Ring', 'Galaxy Ring'] },
  // 睡眠主题
  { cluster: '睡眠健康', topic: 'sleep', keywords: ['睡眠监测', '睡眠质量', '深度睡眠', 'REM睡眠', '睡眠障碍', '改善睡眠', '睡眠分析', '失眠原因'] },
  // OSA主题
  { cluster: 'OSA睡眠呼吸暂停', topic: 'osa', keywords: ['睡眠呼吸暂停', 'OSA筛查', '打鼾原因', 'STOP-BANG量表', '睡眠呼吸暂停症状', 'OSA风险', '呼吸暂停低通气'] },
  // HRV主题
  { cluster: 'HRV心率变异性', topic: 'hrv', keywords: ['心率变异性', 'HRV监测', 'HRV正常值', '心率变异性意义', 'HRV与压力', '自主神经功能'] },
  // 血氧主题
  { cluster: '血氧健康', topic: 'spo2', keywords: ['血氧饱和度', 'SpO2正常值', '血氧低的危害', '夜间血氧监测', '血氧仪', '低氧血症'] },
  // 心脏健康
  { cluster: '心脏健康', topic: 'heart_health', keywords: ['心脏健康监测', '静息心率', '心率异常', '心脏病预防', '心率过快', '心率过慢'] },
  // 压力管理
  { cluster: '压力管理', topic: 'stress', keywords: ['压力监测', '压力指数', '慢性压力危害', '压力与睡眠', 'HRV压力', '减压方法'] },
  // 慢病管理
  { cluster: '慢病数字健康', topic: 'chronic_disease', keywords: ['糖尿病监测', '高血压管理', '慢病数字化', '可穿戴设备慢病', 'GLP-1减重'] },
  // 企业健康
  { cluster: '企业健康管理', topic: 'enterprise', keywords: ['员工健康管理', '企业健康福利', '职场健康', '企业体检', '健康管理平台'] },
];
...
只返回JSON数组，格式：["关键词1","关键词2",...]`;
        ...
        if (match) {
          ...
          for (const kw of expanded.slice(0, 20)) {
            if (kw && kw.length > 2 && kw.length < 50) {
              await this.repo.createKeywordOpportunity({
                keyword: kw, cluster: seed.cluster, topic: seed.topic,
                intent: this.inferIntent(kw), priority: 'medium', contentGap: true,
              }).catch(() => {});
            }
          }
        }
  ...
  async analyzeContentGaps() {
    // 获取所有关键词机会
    const opportunities = await this.repo.listKeywordOpportunities({ status: 'discovered', limit: 200 });
...
请分析：1.重点布局方向 2.内容缺口 3.本周建议优先创作的关键词及原因`;
    ...
    await this.repo.createKeywordReport({
      reportType: 'weekly', periodStart: weekAgo, periodEnd: now,
      title: `关键词机会周报 ${now.toLocaleDateString('zh-CN')}`,
      content, newKeywords: total,
      opportunities: opportunities.slice(0, 10).map(o => ({ keyword: o.keyword, cluster: o.cluster, priority: o.priority })),
    });
  }

  private inferIntent(keyword: string): string {
    if (keyword.includes('什么') || keyword.includes('是什么') || keyword.includes('定义')) return 'informational';
    if (keyword.includes('怎么') || keyword.includes('如何') || keyword.includes('方法')) return 'informational';
    if (keyword.includes('价格') || keyword.includes('多少钱') || keyword.includes('购买')) return 'commercial';
    if (keyword.includes('推荐') || keyword.includes('哪个好') || keyword.includes('比较')) return 'commercial';
