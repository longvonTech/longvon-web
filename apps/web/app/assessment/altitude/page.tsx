...
const QUESTIONS = [
  {
    id: 'destination_altitude',
    text: '您计划前往的海拔高度大约是？',
    options: [
      { label: '2500 米以下', value: 0 },
      { label: '2500 - 3500 米', value: 1 },
      { label: '3500 - 5000 米', value: 2 },
      { label: '5000 米以上', value: 3 },
    ],
  },
  {
    id: 'ascent_speed',
    text: '您计划的上升方式是？',
    options: [
      { label: '逐步适应（多日爬升）', value: 0 },
      { label: '中等速度（1-2 天到达）', value: 1 },
      { label: '快速到达（如直接飞抵高海拔）', value: 2 },
    ],
  },
  {
    id: 'altitude_history',
    text: '此前是否有过明显的高原反应经历？',
    options: [
      { label: '没有', value: 0 },
      { label: '有过轻微不适', value: 1 },
      { label: '有过明显高原反应', value: 2 },
    ],
  },
  {
    id: 'cardio_condition',
    text: '是否有心肺相关基础疾病（如高血压、冠心病、慢阻肺等）？',
    options: [
      { label: '没有', value: 0 },
      { label: '有，但控制良好', value: 1 },
      { label: '有，且近期不够稳定', value: 2 },
    ],
  },
  {
    id: 'physical_fitness',
    text: '您的日常体能水平如何？',
    options: [
      { label: '较好，经常运动', value: 0 },
      { label: '一般，偶尔运动', value: 1 },
      { label: '较弱，很少运动', value: 2 },
    ],
  },
  {
    id: 'stay_duration',
    text: '您预计在高海拔地区停留多长时间？',
    options: [
      { label: '3 天以内', value: 0 },
      { label: '3 - 7 天', value: 1 },
      { label: '7 天以上', value: 2 },
  ...
  },
  {
    ...
    text: '出行前您的整体健康状况如何？',
    options: [
      { label: '良好，无不适', value: 0 },
      { label: '有轻微感冒或疲劳', value: 1 },
      { label: '正在生病或刚康复', value: 2 },
    ],
  },
];
...
const RISK = [
  { max: 2, level: 'low', label: '低风险', color: '#22C55E', bg: '#F0FDF4', desc: '高原相关风险因素较少。建议仍按一般高原旅行注意事项做好准备，关注身体变化。' },
  { max: 5, level: 'moderate', label: '中风险', color: '#F59E0B', bg: '#FFFBEB', desc: '存在一定的高原适应风险。建议安排逐步适应海拔的行程，避免快速上升，并准备基础防护用品。' },
  { max: 9, level: 'high', label: '较高风险', color: '#F97316', bg: '#FFF7ED', desc: '您的高原健康风险较高。建议出行前咨询医生，了解高原反应的预防措施和应急方案。' },
  { max: MAX_SCORE, level: 'very_high', label: '高风险', color: '#EF4444', bg: '#FEF2F2', desc: '您的高原健康风险较高。建议出行前务必咨询医生，评估是否适合前往该海拔地区。' },
];

type Step = 'questions' | 'result' | 'lead';

export default function AltitudeAssessment() {
  ...
  const submitLead = async () => {
    ...
    setSubmitting(true);
    try {
      await fetch('/api/assessment/altitude-lead', {
        method: 'POST',
      ...
      <main style={{ minHeight: '100vh', background: '#FAFAFA', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
        <div style={{ width: '100%', maxWidth: 560 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 32 }}>
            ...
            <span style={{ fontSize: 14, color: '#6B7280' }}>高原健康评估</span>
          </div>
          ...
          <div style={{ background: '#fff', borderRadius: 20, padding: '40px 36px', boxShadow: '0 2px 24px rgba(0,0,0,0.06)', marginBottom: 24 }}>
            <div style={{ fontSize: 13, fontWeight: 600, color: ACCENT, marginBottom: 20, letterSpacing: '0.05em' }}>高原适应风险自评</div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {q.options.map((opt, i) => (
                <button
                  ...
                  onMouseEnter={e => { e.currentTarget.style.background = ACCENT_LIGHT; e.currentTarget.style.borderColor = '#6EE7B7'; }}
          ...
          <p style={{ fontSize: 12, color: '#9CA3AF', textAlign: 'center' }}>结果仅供健康参考，不构成医学诊断。出行前请根据自身情况咨询医生。</p>
        </div>
      </main>
    );
  }
        ...
        <div style={{ width: '100%', maxWidth: 560 }}>
          <div style={{ background: '#fff', borderRadius: 20, padding: '40px 36px', boxShadow: '0 2px 24px rgba(0,0,0,0.06)', marginBottom: 20 }}>
            <div style={{ textAlign: 'center', marginBottom: 32 }}>
              ...
              <div style={{ fontSize: 14, color: '#6B7280', marginBottom: 8 }}>高原健康评估结果</div>
            ...
            <div style={{ marginBottom: 28 }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 6, fontSize: 11, color: '#9CA3AF' }}>
                <span>低 (0-2)</span><span>中 (3-5)</span><span>较高 (6-9)</span><span>高 (10+)</span>
              </div>
            </div>
            ...
            <div style={{ marginBottom: 28 }}>
              <div style={{ fontSize: 14, fontWeight: 600, color: '#1F2937', marginBottom: 12 }}>出行建议：</div>
              <ul style={{ fontSize: 14, color: '#374151', lineHeight: 2, paddingLeft: 20 }}>
                {risk.level === 'low' && (
                  <>
                    <li>了解高原反应常见症状与应对方法</li>
                    <li>保持充足睡眠，避免饮酒和剧烈运动</li>
                    <li>使用 Ring1C 监测血氧与心率变化</li>
                  </>
                )}
                {(risk.level === 'moderate' || risk.level === 'high') && (
                  <>
                    <li>制定逐步适应海拔的行程，避免快速上升</li>
                    <li>准备便携式血氧仪和常用药品</li>
                    <li>出行前咨询医生，了解预防措施</li>
                    <li>使用 Ring1C 实时监测血氧饱和度</li>
                  </>
                )}
                {risk.level === 'very_high' && (
                  <>
                    <li>建议出行前预约医生进行专业评估</li>
                    <li>评估是否适合前往该海拔地区</li>
                    <li>准备完善的应急方案和医疗保障</li>
                    <li>考虑降低目标海拔或延长适应时间</li>
                  </>
                )}
              </ul>
            </div>
        ...
        <div style={{ background: '#fff', borderRadius: 20, padding: '40px 36px', boxShadow: '0 2px 24px rgba(0,0,0,0.06)' }}>
          {leadDone ? (
            <div style={{ textAlign: 'center' }}>
              ...
              <p style={{ color: '#6B7280', fontSize: 14, lineHeight: 1.7, marginBottom: 28 }}>我们的健康顾问将在 24 小时内联系您，提供个性化高原出行建议。</p>
            ...
            <>
              <div style={{ textAlign: 'center', marginBottom: 28 }}>
                ...
                <div style={{ fontSize: 18, fontWeight: 700, marginBottom: 8 }}>获取个性化高原出行方案</div>
              ...
              <button type="button" onClick={submitLead} disabled={submitting || !leadForm.name || !leadForm.phone}
                style={{ width: '100%', padding: '16px', background: submitting ? '#6EE7B7' : ACCENT, color: '#fff', border: 'none', borderRadius: 980, fontSize: 16, fontWeight: 600, cursor: 'pointer' }}>
                {submitting ? '提交中...' : '获取专属出行方案'}
              </button>
