...
const QUESTIONS = [
  ...
  },
  {
    ...
    text: '您的日常饮食习惯如何？',
    options: [
      { label: '较均衡，蔬果蛋白搭配合理', value: 0 },
      { label: '不太规律，经常外食', value: 1 },
      { label: '以高糖、高脂、精加工食品为主', value: 2 },
    ],
  },
  {
    id: 'weight_trend',
    text: '近 6 个月您的体重变化趋势是？',
    options: [
      { label: '基本稳定', value: 0 },
      { label: '反复波动', value: 1 },
      { label: '持续增加', value: 2 },
  ...
  {
    id: 'sleep',
    text: '您每晚实际睡眠时间是否达到 7 小时以上？',
    options: [
      { label: '是，大多数日子如此', value: 0 },
      { label: '偶尔不足', value: 1 },
      { label: '经常不足 6 小时', value: 2 },
    ],
  },
  {
    id: 'sugar_drinks',
    text: '您多久喝一次含糖饮料（奶茶、汽水等）？',
    options: [
      { label: '几乎不喝', value: 0 },
  ...
  {
    id: 'sedentary',
    text: '您每天久坐时间（不含睡眠）大约多长？',
    options: [
      { label: '6 小时以内', value: 0 },
      { label: '6 - 10 小时', value: 1 },
      { label: '10 小时以上', value: 2 },
    ],
  },
  {
    ...
    options: [
      { label: '没有尝试过', value: 0 },
      { label: '尝试过，有一定效果', value: 1 },
      { label: '多次尝试，效果不明显', value: 2 },
    ],
  },
  {
    id: 'meal_regularity',
    text: '您的三餐是否规律？',
    options: [
      { label: '三餐规律，定时定量', value: 0 },
      { label: '偶尔跳过或延迟用餐', value: 1 },
      { label: '经常不规律，常跳早餐', value: 2 },
    ],
  },
];

const RISK = [
  { max: 4, level: 'low', label: '低风险', color: '#22C55E', bg: '#F0FDF4', desc: '您的体重管理风险较低，当前生活方式较为健康。建议继续保持均衡饮食和规律运动。' },
  { max: 9, level: 'moderate', label: '中风险', color: '#F59E0B', bg: '#FFFBEB', desc: '您存在一定的体重管理风险。建议关注饮食结构，增加运动频率，建立健康的生活习惯。' },
  { max: 15, level: 'high', label: '较高风险', color: '#F97316', bg: '#FFF7ED', desc: '您的体重管理风险较高。建议制定具体的饮食与运动计划，必要时咨询营养师或健康管理师。' },
  { max: 24, level: 'very_high', label: '高风险', color: '#EF4444', bg: '#FEF2F2', desc: '您的体重管理风险较高，建议尽快咨询医生或营养专科，制定个性化的体重管理方案。' },
];

type Step = 'questions' | 'result' | 'lead';

export default function WeightLossAssessment() {
  ...
  const submitLead = async () => {
    ...
    setSubmitting(true);
    try {
      await fetch('/api/assessment/weight-loss-lead', {
        method: 'POST',
      ...
      <main style={{ minHeight: '100vh', background: '#FAFAFA', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
        <div style={{ width: '100%', maxWidth: 560 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 32 }}>
            ...
            <span style={{ fontSize: 14, color: '#6B7280' }}>减重风险评估</span>
          </div>
          ...
          <div style={{ background: '#fff', borderRadius: 20, padding: '40px 36px', boxShadow: '0 2px 24px rgba(0,0,0,0.06)', marginBottom: 24 }}>
            <div style={{ fontSize: 13, fontWeight: 600, color: ACCENT, marginBottom: 20, letterSpacing: '0.05em' }}>体重管理风险自评</div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {q.options.map((opt, i) => (
                <button
                  ...
                  onMouseEnter={e => { e.currentTarget.style.background = ACCENT_LIGHT; e.currentTarget.style.borderColor = '#C4B5FD'; }}
        ...
        <div style={{ width: '100%', maxWidth: 560 }}>
          <div style={{ background: '#fff', borderRadius: 20, padding: '40px 36px', boxShadow: '0 2px 24px rgba(0,0,0,0.06)', marginBottom: 20 }}>
            <div style={{ textAlign: 'center', marginBottom: 32 }}>
              ...
              <div style={{ fontSize: 14, color: '#6B7280', marginBottom: 8 }}>减重风险评估结果</div>
            ...
            <div style={{ marginBottom: 28 }}>
              <div style={{ height: 12, background: '#E5E7EB', borderRadius: 6, overflow: 'hidden' }}>
                <div style={{ height: '100%', width: `${(totalScore / 24) * 100}%`, background: risk.color, borderRadius: 6 }} />
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 6, fontSize: 11, color: '#9CA3AF' }}>
                <span>低 (0-4)</span><span>中 (5-9)</span><span>较高 (10-15)</span><span>高 (16+)</span>
              </div>
            </div>
              ...
              <ul style={{ fontSize: 14, color: '#374151', lineHeight: 2, paddingLeft: 20 }}>
                {risk.level === 'low' && (
                  <>
                    ...
                    <li>使用 Ring1C 持续监测日常活动与代谢指标</li>
                    <li>定期关注体重和体脂变化趋势</li>
                  </>
                )}
                {(risk.level === 'moderate' || risk.level === 'high') && (
                  <>
                    <li>减少高糖饮料和精加工食品摄入</li>
                    <li>每周至少 150 分钟中等强度有氧运动</li>
                    <li>建立规律的三餐时间和睡眠习惯</li>
        ...
        <div style={{ background: '#fff', borderRadius: 20, padding: '40px 36px', boxShadow: '0 2px 24px rgba(0,0,0,0.06)' }}>
          {leadDone ? (
            <div style={{ textAlign: 'center' }}>
              ...
              <p style={{ color: '#6B7280', fontSize: 14, lineHeight: 1.7, marginBottom: 28 }}>我们的健康管理顾问将在 24 小时内联系您，提供个性化减重建议。</p>
            ...
            <>
              <div style={{ textAlign: 'center', marginBottom: 28 }}>
                ...
                <div style={{ fontSize: 18, fontWeight: 700, marginBottom: 8 }}>获取个性化减重方案</div>
              ...
              <button type="button" onClick={submitLead} disabled={submitting || !leadForm.name || !leadForm.phone}
                style={{ width: '100%', padding: '16px', background: submitting ? '#C4B5FD' : ACCENT, color: '#fff', border: 'none', borderRadius: 980, fontSize: 16, fontWeight: 600, cursor: 'pointer' }}>
                {submitting ? '提交中...' : '获取专属减重方案'}
              </button>
