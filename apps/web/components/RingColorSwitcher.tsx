...
export function RingColorSwitcher({ compact }: { compact?: boolean }) {
  const [active, setActive] = useState(0);
  if (compact) {
    return (
      <div style={{ textAlign:'center' }}>
        <p style={{ fontSize:11, color:'rgba(255,255,255,0.5)', marginBottom:10, letterSpacing:'0.05em' }}>配色</p>
        <div style={{ display:'flex', gap:10, justifyContent:'center' }}>
          {COLORS.map((c,i) => (
            <button key={c.id} onClick={() => setActive(i)} aria-label={c.label} style={{ width:20, height:20, borderRadius:'50%', background:c.hex, border: i===active ? '2px solid #fff' : '2px solid transparent', cursor:'pointer', padding:0, outline: i===active ? '2px solid rgba(255,255,255,0.3)' : 'none', outlineOffset:2, transition:'all 0.2s' }} />
          ))}
        </div>
        <p style={{ fontSize:11, color:'rgba(255,255,255,0.4)', marginTop:8 }}>{COLORS[active].label}</p>
      </div>
    );
  }
  return (
